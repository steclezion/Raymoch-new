...
<link href="{{ asset('css/style_entire.css')  }}" rel="stylesheet" type="text/css" id="bootstrap">
  ...
  {{-- Your explore JS (moved to a static file) --}}
  <script>
    window.RAYMOCH = {
      API_BASE: "{{ url('/api') }}",           // Laravel proxy
      COMPANIES_URL: "{{ url('/companies') }}" // Laravel companies page
    };
  </script>
    ...
    <section class="container">
      ...
      <form class="search-panel card" role="search" id="searchForm">
        ...
        <div class="tier tier-3" style="margin-top:6px">
          <a class="btn" id="allCompaniesBtn" href="{{ url('/company') }}">All Companies</a>
        </div>
      </form>
