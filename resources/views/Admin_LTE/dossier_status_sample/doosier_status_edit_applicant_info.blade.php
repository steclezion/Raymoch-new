...
<div class="row">
    ...
    <div class="col-sm-9">
            ...
            Placeholder="Please zip the entire dossier and paste single link "   value="{{ $app->dossier_url}}"/>
    </div>
    <div class="col-sm-3">
        <label>Sample Submitted?</label>  
        </div>
        <div class="col-sm-9">
        @php if($app->sample_status == 1) {$value="checked"; } else {$value="";}   @endphp 
            <input style="width:20px;position:float:left;"  {{ $value }} class="form-control" id="sample_status" type="checkbox" name="sample_status"  />
          </div>
           </div>
           @endforeach
