...
<div  class="modal fade" id="ajaxModel" aria-hidden="true">
    ...
    <form id="bookForm"    enctype="multipart/form-data"  action="{{ route('receipts.store') }}" method="POST" name="bookForm" class="form-horizontal"   >
              ...
              <input type="hidden" name="_token" value="{{ csrf_token() }}" />
                   ...
                   <!--
                   <select class="form-control select2bs4" style="width: 100%;"  name="generated_number_invoice" id="generated_number_invoice"  required  >
                   <option  selected="true"></option> 
                   @foreach ($invoice_generated as $key => $value)
                   <option value="{{$value->invoice_number}}">{{  $value->invoice_number."---". $value->first_name." ".$value->middle_name." ".$value->last_name   }}  </option>
                   @endforeach
                   </select>-->
