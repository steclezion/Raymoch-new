...
export default function Login({
  ...
  // (Your UI guard is fine; keep it if you want)
  useEffect(() => {
    ...
    const lockBg = (el, name) => {
      ...
      const handler = () => {
        ...
        if (currentBg !== initialBg) {
          console.warn(`[UI GUARD] ${name} background changed on hover. Resetting.`, {
            initialBg,
            currentBg,
          });
...
.page::before{
  ...
  position:fixed;
  inset:0;
  background-image:url('${africaMapUrl}');
...
.page::after{
  ...
  position:fixed;
  inset:0;
  background: radial-gradient(circle at center, rgba(255,255,255,0.85), rgba(255,255,255,1));
  z-index:0;
  pointer-events:none;
}
