    ...
    async function loadCodes() {
      ...
      try {
        ...
        if (!res.ok || !json.ok || !Array.isArray(json.data)) {
          toast.error(json?.message || "Unable to load country codes.");
          setCcOptions([]);
          return;
        }
  ...
  const selectStyles = useMemo(
    () => ({
      ...
      option: (base, state) => ({
        ...
        padding: "10px 12px",
        fontWeight: 700,
        backgroundColor: state.isSelected ? "#eef2ff" : state.isFocused ? "#f1f5ff" : "#fff",
        color: "#0f172a",
      }),
    }),
    []
          ...
          <section className="card">
            <div className="cardHeader">
              <div>
                ...
                <p className="cardSub">Create a basic account and verify your email in seconds.</p>
              </div>

              <button type="button" className="btn btnGhost" onClick={() => window.history.back()}>
                Back
              </button>
            </div>
            ...
            {step === "form" && (
              <form noValidate onSubmit={sendOtp} className="sbForm">
                    ...
                    <div className="phoneRow">
                      <div className="phoneDial">
                        <Select
                          ...
                          value={selectedCC}
                          isLoading={ccLoading}
                          isDisabled={ccLoading || ccOptions.length === 0}
                          onChange={(opt) => setCountryCode(opt?.dial || countryCode)}
                          getOptionValue={getOptionValue}
                          getOptionLabel={getOptionLabel}
                          isSearchable
                          menuPlacement="auto"
                          placeholder={ccLoading ? "Loading…" : "Code"}
                ...
                <div className="actions">
                  <button type="submit" className="btn btnPrimary cta" disabled={busy} aria-disabled={busy}>
