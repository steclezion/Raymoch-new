...
/* SweetAlert2 mixin (compact, animated, brand-styled) */
...
const swalSuccess = (msg, timer = 1500) =>
  swal.fire({ icon: "success", title: msg, timer, showConfirmButton: false });
const swalError = (msg) =>
  swal.fire({ icon: "error", title: "Oops", text: msg, confirmButtonText: "OK" });
const swalInfo = (msg, timer = 1400) =>
  swal.fire({ icon: "info", title: msg, timer, showConfirmButton: false });
...
function PremiumForm({ routes }) {
  ...
  const validate = () => {
    if (!values.name.trim()) return swalError("Enter your full name."), false;
    if (!/\S+@\S+\.\S+/.test(values.email.trim())) return swalError("Enter a valid email."), false;
    if (!values.display_name.trim()) return swalError("Enter a display name."), false;
    if (!values.password || !strongPass(values.password))
      return swalError("Password must be ≥9 chars, include 1 uppercase, 1 number, 1 special."), false;
    if (values.password !== values.confirm_password)
      return swalError("Passwords do not match."), false;
    if (!values.company_name.trim())
      return swalError("Enter your company (or N/A)."), false;
    if (!values.consent) return swalError("You must agree to the Terms & Privacy."), false;
    return true;
  };
  ...
  const checkEmail = async (email) => {
    ...
    try {
      ...
      if (taken) {
        await swalError("This email is already registered. Try signing in or use another email.");
      }
      return taken;
    } catch {
      ...
      const tick = () => {
        ...
        if (remaining === 0) {
          swal.fire({
            icon: "error",
            title: "Code expired",
            text: "Your verification code expired. The page will reload.",
            timer: 1500,
  ...
  const sendOtp = async (e) => {
    ...
    try {
      ...
      const json = await res.json().catch(() => ({}));
      if (res.ok && json.ok) {
        swalSuccess("Verification code sent to email.");
        ...
        swalError(json?.message || "Unable to send code.");
      }
    } catch {
  ...
  // ===== Helpers to trigger verify on last digit keyup/keydown =====
  ...
  const verifyOtp = async () => {
    const code = otp.join("");
    if (code.length !== 6) return swalError("Enter the 6-digit code.");
    setBusy(true);
    try {
      ...
      const json = await res.json().catch(() => ({}));
      if (res.ok && json.ok) {
        swalSuccess("Email verified. Enter card to activate premium.");
        setStep("pay");
      } else {
        swalError(json?.message || "Invalid code.");
      }
    } catch {
      swalError("Network error.");
    } finally {
  ...
  const handlePay = async () => {
    ...
    if (!billing.name || !billing.email || !billing.postal_code || !billing.country) {
      return swalError("Please complete required billing fields (Name, Email, Postal, Country).");
    }

    setBusy(true);
    try {
      // 1) Create a $9 PaymentIntent on server
      const piRes = await fetch(R.payment?.create_payment_intent ?? "/payment/create-payment-intent", {
        method: "POST",
        headers: {
          "X-CSRF-TOKEN": csrf,
          Accept: "application/json",
        ...
        body: JSON.stringify({
          ...
          metadata: { plan: "premium", signup_email: values.email },
        }),
      });
      ...
      const { error, paymentIntent } = await stripe.confirmCardPayment(piJson.client_secret, {
        payment_method: {
          card,
          billing_details: {
            name: billing.name,
      ...
      const finalRes = await fetch(R.signup?.premium?.complete ?? "/signup/premium/complete", {
        method: "POST",
        headers: {
          "X-CSRF-TOKEN": csrf,
          Accept: "application/json",
      ...
      if (!finalRes.ok || !finalJson.ok) {
        throw new Error(finalJson?.message || "Could not finalize premium account.");
      }

      await swalSuccess("Payment received. Premium account activated!", 1700);
      window.location.assign(finalJson.redirect || "/dashboard");
    } catch (err) {
      swalError(err.message || "Payment error.");
    } finally {
      setBusy(false);
    }
  };
  ...
  return (
    <>
      {/* Base styles + modal + SweetAlert custom styles */}
      <style>{css}</style>
          ...
          <section className="card">
              ...
              <form noValidate onSubmit={sendOtp}>
                ...
                <div className="consent">
                  ...
                  <label htmlFor="p-consent">
                    ...
                    <br />
                    <small>We’ll use your info to create a premium account and send essential updates.</small>
                  </label>
                </div>
            ...
            {step === "pay" && (
              <div style={{ marginTop: 8 }}>
                <h2 className="otpTitle">Card for Premium (one-time $9.00)</h2>
                <p className="muted">We’ll charge $9.00 now to activate your premium account.</p>
                ...
                <div className="actions" style={{ marginTop: 12 }}>
                  <button type="button" className="cta" onClick={handlePay} disabled={busy || !stripe}>
                    {busy ? "Processing…" : "Pay $9.00 & Activate"}
                  </button>
                </div>
              </div>
            )}
