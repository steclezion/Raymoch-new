// resources/js/pages/PricingBasic.jsx
...
export default function PricingBasic({ routes = {} }) {
  ...
  const basicCreate =
    routes?.signup?.basic?.create?.individual || "/signup/basic/create/individual";
  const premiumCreate =
    routes?.signup?.premium?.create || "/signup/premium/create/individual";

  const signupIndex = routes?.signup?.index || "/signup";

  // Footer safe fallbacks (so Footer links never crash)
  const r = {
    ...routes,
  ...
  return (
    <div className="page">
      ...
      {/* ✅ Import + use Header as you requested */}
      <Header routes={r} />
      ...
      {/* ✅ Import + use Footer as you requested */}
      <Footer routes={r} />
    </div>
  );
}
...
/* ✅ IMPORTANT:
   Header.jsx already injects .header/.brand/.menu styles via <style>{header_csss}</style>
   So DO NOT redefine .header/.brand/.menu here, otherwise you’ll fight CSS conflicts.
*/
...
.plan[data-selected="true"]{
  border-color:#c9d4ff;
  box-shadow:0 0 0 3px rgba(33,59,177,.15)
}
